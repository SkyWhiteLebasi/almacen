...
class PedidoController extends Controller
{
    ...
    {
        $datos['pedidos']=Pedido::paginate(25);
        $med=Medida::all();
        $produc=Producto::all();
        $semana=Semana::all();
        return view('pedido.index',$datos, compact('med', 'produc','semana') );
    }
    ...
    public function store(Request $request)
    {
        if($request->hasFile('import_file')){ //ára traerme la foto
            ...
            return redirect()->route('pedido.index')->with('success', 'pedidos importados exitosamente');
        }
